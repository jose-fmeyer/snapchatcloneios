...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    override func viewDidLoad() {
        ...
        delegate.shouldEnableCaptureModeControlUI(enabled: false)
        
        // Set up the video preview view.
        previewView = delegate.previewViewInstance()
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if context == &sessionRunningObserveContext {
            ...
            DispatchQueue.main.async { [unowned self] in
                ...
                self.delegate.shouldEnableCaptureModeControlUI(enabled: isSessionRunning)
            }
        }
        else {
        ...
        if error.code == .mediaServicesWereReset {
            sessionQueue.async { [unowned self] in
                if self.isSessionRunning {
                    ...
                    DispatchQueue.main.async { [unowned self] in
                        self.delegate.shouldHiddenResumeButton(hidden: false)
                    }
                }
            }
        }
    ...
    func sessionWasInterrupted(notification: NSNotification) {
        ...
        if let userInfoValue = notification.userInfo?[AVCaptureSessionInterruptionReasonKey] as AnyObject?, let reasonIntegerValue = userInfoValue.integerValue, let reason = AVCaptureSessionInterruptionReason(rawValue: reasonIntegerValue) {
            ...
            if reason == AVCaptureSessionInterruptionReason.audioDeviceInUseByAnotherClient || reason == AVCaptureSessionInterruptionReason.videoDeviceInUseByAnotherClient {
                ...
                delegate.configureCameraUnavailableLabelVisibility(alpha: 0)
                delegate.shouldHiddenCameraAvailableLabel(hidden: false)
                UIView.animate(withDuration: 0.25) { [unowned self] in
                    self.delegate.configureCameraUnavailableLabelVisibility(alpha: 1)
                }
            }
            
            if showResumeButton {
                // Simply fade-in a button to enable the user to try to resume the session running.
                delegate.configureResumeButtonVisibility(alpha: 0)
                delegate.shouldHiddenResumeButton(hidden: false)
                UIView.animate(withDuration: 0.25) { [unowned self] in
                    self.delegate.configureResumeButtonVisibility(alpha: 1)
                }
            }
        }
    }
    ...
    func sessionInterruptionEnded(notification: NSNotification) {
        ...
        if !delegate.isResumeButtonHidden() {
            UIView.animate(withDuration: 0.25,
                           animations: { [unowned self] in
                            self.delegate.configureResumeButtonVisibility(alpha: 0)
                }, completion: { [unowned self] finished in
                    self.delegate.shouldHiddenResumeButton(hidden: true)
                }
            )
        }
        if !delegate.isCameraUnavailableLabelHidden() {
            UIView.animate(withDuration: 0.25,
                           animations: { [unowned self] in
                            self.delegate.configureCameraUnavailableLabelVisibility(alpha: 0)
                }, completion: { [unowned self] finished in
                    self.delegate.shouldHiddenCameraAvailableLabel(hidden: true)
                }
            )
        }
    }
    ...
    func toggleCaptureMode() {
        if delegate.captureModeControlSelectedIndex() == CaptureMode.photo.rawValue {
            sessionQueue.async { [unowned self] in
                /*
        ...
        else if delegate.captureModeControlSelectedIndex() == CaptureMode.movie.rawValue
        {
